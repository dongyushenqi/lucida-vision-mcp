...
## Two Distribution Forms (Important)

| Form | For whom | What it is | How to use |
|---|---|---|---|
| **Source zip** (auto-generated by GitHub) | Developers (already have a toolchain) | All source + tests + CI, ~124 KB | `pnpm install && pnpm build` then run |
...
The release package includes launchers for Windows (`install.cmd`/`start.cmd`) and
macOS/Linux (`install.sh`/`start.sh`), plus bilingual instructions.
