...
- 提供 10 个工具：`vision.observe`（观察）、`vision.summarize`（1~16 张图批量综合概述，覆盖不全时如实声明范围）、`vision.ocr`（文字提取）、`vision.detect`（结构化检测）、`vision.session.*`（会话与审计）、`vision.operation.*`（任务查询/取消）。
...
- **诚实与推断（平衡版）**：观察不到的不编造；基于明显视觉证据的合理推断可带「看起来/可能是」等限定词表述（如光线明亮→可能是白天）；无视觉依据的精确判断（具体拍摄时间、地点名）不输出；气质等轻推断可带限定表达；`vision.summarize` 若未能覆盖全部图片，会如实声明实际覆盖范围；
