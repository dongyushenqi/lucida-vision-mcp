...
- 指令校准（实测反馈）：① 新增**环境推断禁令**——不得推断拍摄时间（午后/傍晚）、地点、季节、天气成因等，除非画面中存在可直接读取的证据（钟表、文字招牌）；光线与阴影本身可描述为可观察事实（此前 summarize 输出出现过"午后的阳光"式虚构）；② summarize 输出改为**平实事实化表述**——像工作摘要一样陈述事实，禁止文学化修辞、氛围渲染与抒情（"散文化"表述已从全部代码与文档中清除，统一为"平实连贯的综合描述"）；③ 工具描述加**多图引导**——observe 描述明确"需要综合理解多张图片时，请优先使用 vision.summarize"。
- 测试 205 个全绿（三平台 CI）。

**怎么用 / How to use**

- 无需配置变更；重启宿主后生效。让 AI"概括一下这批图"→ `vision.summarize` 输出平实综合描述；需要细节用 `vision.observe`。
