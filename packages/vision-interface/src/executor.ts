...
/**
 * 默认综合概述指令（仅当 Agent 未提供 instruction 时使用；平实连贯的段落式表述，{n} 为图片张数）。
 * 与 observe 校准口径一致：客观特征必述 / 忽略水印等细碎元素 / 主观评价三档 / 不推断不编造。
 */
export const DEFAULT_SUMMARIZE_INSTRUCTION =
  "综合概述这批图片（共 {n} 张）：提炼共同主题、显著差异与整体印象。" +
  "若未能覆盖全部 {n} 张图片，必须在输出开头明确说明实际覆盖范围" +
  "（如“仅覆盖第 1~4 张”），不得假装已分析全部图片；" +
  "用平实自然的语言陈述事实，避免文学化修辞、氛围渲染与抒情；保持可读性，不必刻意干燥，不要逐张罗列每张图的细节。" +
  "只描述可观察的客观特征，必须如实给出；主体是什么就描述什么（人物/风景/物品等）。" +
  "默认忽略水印与细小标识的文字转录、无关背景细节。\n" +
  "不推断拍摄时间（如午后/傍晚）、地点、季节或天气成因，除非画面中存在可直接读取的证据（如钟表、文字招牌）；" +
  "光线与阴影本身可以描述为可观察事实。\n" +
  "主观评价（美丑、性感、魅力等）因人而异，但存在多数人的审美共识：" +
  "默认不主动输出；当用户明确要求时，基于可观察的客观特征给出评价，" +
  "可参考普遍审美标准，并说明支撑评价的可见依据；分歧空间可如实提及，但不得以“主观”为由拒绝回答。\n" +
  "不编造：无视觉依据的内容不得输出。基于明显视觉证据的合理推断可以表述，" +
  "但必须用“看起来”“可能是”等限定词标注不确定性" +
  "（如：光线明亮→可能是白天；短袖、遮阳伞→衣着显示可能为暖季）。" +
  "具体拍摄时间（如“午后”）、具体地点名称（如“北京街头”）除非画面有可直接读取的证据" +
  "（钟表、文字招牌），否则不得输出；" +
  "可观察的动作、姿态与表情必须如实描述；基于整体视觉印象的轻推断（如气质、态度）可以表达，" +
  "但应带“看起来”“给人感觉”等限定，不得作为确定事实断言；" +
  "职业、健康状况、拍摄意图等推断一律不输出。";
...
export const DEFAULT_DEEP_SUMMARIZE_INSTRUCTION =
  ...
  "若未能覆盖全部 {n} 张图片，必须在输出开头明确说明实际覆盖范围" +
  "（如“仅覆盖第 1~4 张”），不得假装已分析全部图片；" +
  "用平实自然的语言陈述事实，避免文学化修辞、氛围渲染与抒情；保持可读性，不必刻意干燥，" +
  "可以分主题组织，但不要为每张图单独罗列段落。" +
  "只描述可观察的客观特征，必须如实给出。\n" +
  "不推断拍摄时间（如午后/傍晚）、地点、季节或天气成因，除非画面中存在可直接读取的证据（如钟表、文字招牌）；" +
  "光线与阴影本身可以描述为可观察事实。\n" +
  "主观评价（美丑、性感、魅力等）因人而异，但存在多数人的审美共识：" +
...
export class VisionExecutor {
  ...
  private buildObservations(
    ...
    // summarize：批量综合概述 → 单个平实事实化的综合 Observation（label=summary）
    if (kind === "summarize") {
      return [
        this.toObservation(
          base,
