 ...
 * - 关闭语义（审查 #5）：closed 后拒绝新的执行类操作；读取保留证据（session.get、
 *   resources/read）允许（allowClosed）。
 */
...
export interface AuthorizeOptions {
  /** 允许访问已关闭（closed）的 Session（仅限读取保留证据类操作） */
  allowClosed?: boolean;
}
...
export class SessionSandbox {
  ...
  authorize(sessionId: string, identity: IdentityContext, opts: AuthorizeOptions = {}): VisionSession {
    ...
    if (session.status === "closed" && !opts.allowClosed) {
      throw new VisionError(ApplicationErrorCode.SESSION_CLOSED, "Vision Session 已关闭，不允许新的执行操作", {
        vision_session_id: sessionId,
      });
    }
    return session;
